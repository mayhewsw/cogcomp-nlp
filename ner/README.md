...
This relies on a [special version of LBJava](https://github.com/mayhewsw/lbjava/tree/weighted) that allows weights while training. The best way to get it is to do git checkout and install (sorry). Make sure you are installing the `weighted` branch. The pom.xml in this directory depends on that library, so everything should go swimmingly once it is installed.

The main script to use is [loreleirunner.sh](scripts/loreleirunner.sh). Just change the TRAIN/TEST/CONFIG variables in the script, and re-run. Include the `--outpath` option if you want to write out the annotated test files. 
